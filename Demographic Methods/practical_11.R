...
# Not so good:  
# The main purpose of questions 1 and 2 is simply to determine eligibility for questions 3–10. However, these questions could probe for the actual places.  
# The questionnaire is designed to identify who is a migrant and since when; however, it is not particularly effective at probing for the place of birth or previous residence (e.g., it only captures broad regions and emphasizes size rather than the actual location).  
# It is preferable to ask for the actual place and determine whether it is rural or urban, rather than allowing respondents—or enumerators—to decide if a place is small, medium, large, or a village.  
# Although question 3 could be used to estimate lifetime migration, it would be preferable to ask for the mother’s place of residence at the time of birth, given that people may migrate specifically for childbirth. This is particularly common in low- and middle-income countries, where expectant mothers often move to access medical care or family support.  
...
# Different approach:
# I would define a retrospective period of 1 or 5 years before the date of the interview, and I would probe for the number of movements, previous places of residence (name of local government, municipality, state, and urban/rural), and the associated dates (or ages) of migration.  
