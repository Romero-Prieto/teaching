...
TFRc                 = cohort[rEP == 1, list(TFR = sum(NnFx, na.rm = TRUE), mu = sum(mu, na.rm = TRUE)), by = list(Cohort)]
TFRc[["mu"]]         = TFRc[["mu"]]/TFRc[["TFR"]]
TFRc
...
TFR                  = period[rEP == 1, list(TFR = sum(NnFx, na.rm = TRUE), mu = sum(mu, na.rm = TRUE)), by = list(Year)]
TFR[["mu"]]          = TFR[["mu"]]/TFR[["TFR"]]
TFR
...
TFRc[["Year"]]       = TFRc[["Cohort"]] + TFRc[["mu"]]                          #Generates a column "Year" as a combination of the cohort and mu. 
TFRc[["Analysis"]]   = "Cohort"                                                 #Generates a column "Analysis" indicating that this table corresponds to cohort analysis.

TFR[["Year"]]        = TFR[["Year"]] + 0.5                                      #Adjusts the "Year" column to represent the midpoint of each calendar year.
TFR[["Analysis"]]    = "Period"                                                 #Generates a column "Analysis" indicating that this table corresponds to period analysis.
Fertility            = data.frame(rbind(TFRc[, c("Year", "TFR", "mu", "Analysis")], TFR)) #Appends the two tables into a single data frame named "Fertility", which is advantageous for plotting.

coloUR               = c("Cohort" = rgb(0.00,0.55,0.65,0.35),
                         "Period" = rgb(0.45,0.65,0.20,0.65))                   #Creates a matrix of arbitrary colors—one per type of analysis.

ggplot(Fertility, aes(x = Year, y = TFR, color = Analysis, group = Analysis)) +
  geom_line(linewidth = 1.5) +
  coord_cartesian(ylim = c(0, 4.5)) +
  scale_color_manual(values = coloUR) +
  scale_x_continuous(breaks = seq(1900, 2020, by = 10)) +
  scale_y_continuous(breaks = seq(0, 4.5, by = 0.5)) +
  labs(title = "Total Fertility Rate", x = "Year", y = "Children per woman") + 
  theme_minimal()                                                               #Plots the TFR and the TFRc. 
...
fertility = function(data){                                                     #Creates a function named fertility and using data as the only input.
  summary              = data[, list(Wi = sum(W, na.rm = TRUE)), by = list(Parity)][order(Parity)] #Collapse and sort the data, adding the number of women for each parity.
  PPR                  = data.table(Parity = 0:max(data[["Parity"]]), Wi = rep(0,max(data[["Parity"]]) + 1)) #Creates a frame which is including all possible parities (from zero to the reported maximum number of children ever born).
  setDT(PPR)[summary, on = "Parity", c("Wi") := list(i.Wi)]                     #Merges and updates the PPR table using matching information from the summary table.
  PPR[["Pi"]]          = rev(cumsum(rev(PPR[["Wi"]])))                          #Quantifies the number of women with parity less than or equal to i (Pi).
  ...
  TFRc                 = sum(tail(PPR[["PPRoi"]], -1))                          #Calculates the TFRc.
  return(list(PPR = PPR, TFRc = TFRc))                                          #Defines the outputs of the function: the PPR table and the resulting TFRc. 
}

Residence            = c("Urban: ", "Rural: ", "Gap: ")                         #Defines the labels for the results.
TFRc                 = rep(NA, 3)                                               #Defines a frame for the results.
for (i in 1:2){                                                                 #Uses a loop to apply the same commands to different populations.
  result               = fertility(data[data[["UR"]] == i])                     #Evaluates the function, using a subset of data (e.g., one per place of residence).
  TFRc[i]              = result[["TFRc"]]                                       #Allocates the resulting TFRc. 
}
TFRc[3]              = TFRc[2] - TFRc[1]                                        #Calculate and stores the fertility gap.
print(data.frame(Residence, TFRc), row.names = FALSE)                           #Print the results as a labeled table.
...
Education            = c("Less than complete primary: ",
                         "Incomplete secondary: ", 
                         "Complete secondary or higher: ")                      #Defines the labels for the results.
TFR                  = rep(NA, 2)                                               #Defines a frame for the results.
for (i in 1:3){                                                                 #Uses a loop to apply the same commands to different populations.
  result               = fertility(data[data[["Education"]] == i])              #Evaluates the function, using a subset of data (e.g., one per level of education).      
  TFRc[i]              = result[["TFRc"]]                                       #Allocates the resulting TFRc.
}
print(data.frame(Education, TFRc), row.names = FALSE)                           #Print the results as a labeled table.